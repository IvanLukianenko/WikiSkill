...
5. Mark the sampled traces done: `python3 .wikiskill/bin/wikiskill.py mark-consolidated <sampled digest paths>` (use `--all` only if you actually analyzed all pending traces).
6. Report: patterns created/updated and the key root causes found. If the wiki gained substantive knowledge, suggest `/wikiskill:evolve` next.
