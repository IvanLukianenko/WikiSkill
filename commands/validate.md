...
**Baseline mode** — if the argument is `--baseline`, or status shows "not baselined yet": run the suite with the current skill set and record `python3 .wikiskill/bin/wikiskill.py record-validation --baseline --passed <M> --total <N> --results "VT-1=pass,VT-2=fail,..." --note "baseline"`. This initializes R_best (the paper's R(T_val,0)); report and stop.

Always pass `--results` with every task's outcome — it feeds the suite lifecycle (pass streaks → saturation → retirement), which is what keeps validation cost bounded.
...
2. Record and gate: `python3 .wikiskill/bin/wikiskill.py record-validation --skill <name> --passed <M> --total <N> --results "VT-1=pass,VT-2=fail,..." --note "<one-line context>"`. The CLI compares against R_best, appends the proposal's unified diff and outcome to `wiki/skill-impact.md`, and advances the iteration counter. If it exits with **SUITE CHANGED** (the suite was modified since R_best was anchored — pass rates across different suites are not comparable), do what it says: roll the skill back temporarily or note its state, run the full suite as a fresh `--baseline` with the pre-proposal skill set (rollback → baseline → re-apply from the snapshot diff is the clean order; if impractical, baseline with the proposal applied and say so in the note), then re-run this gating step.
