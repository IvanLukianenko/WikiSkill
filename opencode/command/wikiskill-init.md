...
1. Run: `python3 .opencode/wikiskill/wikiskill.py init`
2. Read `.opencode/wikiskill/METHODOLOGY.md` and briefly explain the three layers
   (Raw traces / persistent Wiki with patterns+index+log+skill-impact / gated
   Skills with SKILL.md+PURPOSE.md) to the user.
3. Recommend adding validation tasks to `.wikiskill/validation/tasks.md` (show
   the template format from that file) — the suite is the paper's D_val.
4. Explain the workflow: work normally — the WikiSkill opencode plugin captures
   traces automatically — then periodically run `/wikiskill-loop` for one
