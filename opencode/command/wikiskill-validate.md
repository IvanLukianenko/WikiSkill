...
`.opencode/wikiskill/METHODOLOGY.md`. Check state first:
`python3 .wikiskill/bin/wikiskill.py status`.

**Baseline mode** (argument `--baseline`, or status shows "not baselined yet"):
run the suite in `.wikiskill/validation/tasks.md` with current skills, then
`python3 .wikiskill/bin/wikiskill.py record-validation --baseline --passed <M> --total <N> --note "baseline"`.
